...
# FIX 9: ^VIX und ^VVIX benötigen das Caret-Präfix für yfinance
STOCK_GROUPS = {
...
def get_stock_data(ticker_symbol, charts_dir):
    try:
        ...
        # FIX 10: RRS nur für echte ETFs/Aktien – nicht für Indizes (^VIX, ^VVIX)
        if not is_index(ticker_symbol):
            now         = now_berlin()
...
def main():
    ...
    snapshot = {
        # FIX 7: ISO-Zeitstempel mit MEZ/MESZ-Offset
